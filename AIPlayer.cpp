...
int AIPlayer::dfs(ChessPiece& maxP, int d, ChessPad& pad, int pid, int alpha,
                  int beta) {
...
int AI::evaluate(ChessPad& pad, const ChessPiece& p) {
    int typ[11] = {}, ret = 0;
    // pad.place(p);
    for (int i = 0; i < 4; i++) ++typ[pad.getType(p, i)];
    // pad.remove();
    for (int i = 0; i < 11; i++) ret += typ[i] * E_SCORE[i];
    return ret;
}
    ...
    for (int8_t x = 0; x < 15; x++)
        for (int8_t y = 0; y < 15; y++) {
            ...
            if (!pad[x][y] || pid == 2) {
                ...
                if (hiScore >= E_SCORE[WIN5])
                    win5.push_back(p);
                else if (hiScore > E_SCORE[ALIVE4])
                    ualive4.push_back(p);
                else if (hiScore > E_SCORE[DIE4])
                    udie4.push_back(p);
                else if (hiScore > 2 * E_SCORE[ALIVE3])
                    udouble3.push_back(p);
                else if (hiScore > E_SCORE[ALIVE3])
                    ualive3.push_back(p);

                p.set(pid, x, y);  // 评估我方的分数
                int myScore = evaluate(chessPad, p);
                if (myScore >= E_SCORE[WIN5])
                    win5.push_back(p);
                else if (myScore > E_SCORE[ALIVE4])
                    valive4.push_back(p);
                else if (myScore > E_SCORE[DIE4])
                    vdie4.push_back(p);
                else if (myScore > 2 * E_SCORE[ALIVE3])
                    vdouble3.push_back(p);
                else if (myScore > E_SCORE[ALIVE3])
                    valive3.push_back(p);
                // 否则为普通棋子，加入排序数组
                else
                    vec.push_back({hiScore - myScore, p});
            }
        }

    if (!win5.empty())
        v = win5;  // 如果可以连5，只考虑连5点
    else if (!valive4.empty())
        v = valive4;  // 我方可以活4
    else if (!ualive4.empty() && vdie4.empty())
        v = ualive4;  // 对方可以活4，我方不能冲4，就去堵他
        ...
        if (udouble3.empty() && vdouble3.empty()) {
            ...
            for (int i = 0; i < n; i++) v.push_back(vec[i].second);
        }
    }
}
...
int AI::g(int pid, const ChessPad& chessPad) {
    ...
    // 我们没有冲4 但对面有活3 对方下一步就要来活4 已经输了
        ...
        a -= 60000;
    // 我们两个活3 对面没4也没活3
    // 也就是说这一步他们最多造个3出来（或者堵我们一个3） 下一步我们就能活4
    // 对面却不能活4
